...
import { getMdxContentMeta } from "./mdx/getMdxContentMeta";
...
export function getProjects() {
  ...
  return [...restructuredProjects].sort((a, b) => (b.id < a.id ? -1 : 1));
}
...
export function getProjectMeta(slug) {
  const { frontmatter } = getMdxContentMeta("projects", slug);
  return frontmatter;
}
...
export function getBlogs() {
  ...
  return [...restructuredBlogs].sort((a, b) => (b.id < a.id ? -1 : 1));
}
